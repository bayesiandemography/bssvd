...
#'
#' When values are being treated as probabilities,
#' a logit transform is applied before
#' carrying out the SVD. When they are being
#' treated as rates, a log transform is applied.
#'
...
#' 
#' @examples
#' zipfile <- system.file("extdata", "himd_subset.zip",
#'                        package = "bssvd")
#' data <- data_ssvd_himd(zipfile)
#' data
#' @export
...
#' 
#' @noRd
himd_calculate_coef <- function(data, measure_type, n_comp) {
...
#'
#' @noRd
himd_total <- function(data, n_comp, transform) {
...
himd_unzip <- function(zipfile, time_interval) {
  ...
  dir_files <- file.path(tmp_dir, base_zipfile)
  fnames <- list.files(dir_files, full.names = TRUE)
